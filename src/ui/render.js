...
function resetCrmLoadedDataForRangeChange() {
  ...
  state.smsQueryCache = new Map();
  state.phoneCache = new Map();
...
function updateButtons() {
  ...
  if (els.accountCol) els.accountCol.disabled = state.running;
  if (els.eventCol) els.eventCol.disabled = state.running;
}
...
function buildResultRowHtml(item) {
  ...
  return `
    <tr class="${rowClass}">
      ...
      ${buildCompactCell(item.smsSendTime || '')}
