...
(function (A) {
  ...
  <style>
    ...
    * { box-sizing: border-box; cursor: var(--jdbean-cursor-arrow), default; }
    .panel {
      ...
      z-index: 2147483647; width: 980px; max-width: calc(100vw - 28px); max-height: calc(100vh - 56px);
      overflow: auto; background: #fff; color: #1d1d1f; contain: layout paint style;
      border: 1px solid rgba(255,255,255,.62); border-radius: 26px;
      box-shadow: 0 16px 48px rgba(0,0,0,.20), 0 2px 10px rgba(0,0,0,.08);
      padding: 14px; font: 13px/1.45 -apple-system, BlinkMacSystemFont, "SF Pro Text", "Segoe UI", "Microsoft YaHei", Arial, sans-serif;
    }
    :host(.app-mode) { position: fixed; inset: 0; z-index: 2147483647; width: 100vw; height: 100vh; display: block; background: #f5f5f7; }
    :host(.app-mode) .panel {
      position: fixed; inset: 0; top: 0 !important; left: 0 !important; transform: none !important;
      width: 100vw !important; max-width: none !important; height: 100vh !important; max-height: none !important;
      border: 0; border-radius: 0; box-shadow: none; padding: 18px 22px; overflow: auto; background: #f5f5f7;
    }
    :host(.app-mode) .runtime-title { margin: -18px -22px 0; border-radius: 0; padding: 10px 22px; }
    :host(.app-mode) .header { margin: 0 -22px 14px; padding: 16px 22px 14px; cursor: var(--jdbean-cursor-arrow), default; }
    :host(.app-mode) .window-dots, :host(.app-mode) .toggle, :host(.app-mode) .restore-pill { display: none !important; }
    ...
    :host(.app-mode) .table-wrap { max-height: calc(100vh - 430px); min-height: 260px; }
    .runtime-title { margin: -14px -14px 0; padding: 9px 16px 8px; border-radius: 26px 26px 0 0; border-bottom: 1px solid rgba(0,0,0,.08); background: linear-gradient(180deg, rgba(255,255,255,.86), rgba(245,245,247,.78)); color: #6e6e73; font-size: 12px; line-height: 1.35; white-space: nowrap; overflow: hidden; text-overflow: ellipsis; user-select: none; display: flex; align-items: center; gap: 10px; }
    .runtime-title-text { flex: 1 1 auto; min-width: 0; overflow: hidden; text-overflow: ellipsis; white-space: nowrap; }
    .header { cursor: var(--jdbean-cursor-arrow), move; user-select: none; margin: 0 -14px 12px; padding: 13px 16px 12px; border-bottom: 1px solid rgba(0,0,0,.08); background: linear-gradient(180deg, rgba(248,248,250,.96), rgba(238,238,241,.92)); display: flex; justify-content: space-between; gap: 10px; align-items: center; border-radius: 0; }
    ...
    .source-summary { max-width: 600px; overflow: hidden; text-overflow: ellipsis; white-space: nowrap; color: #6e6e73; font-size: 12px; }
    .toggle { height: 28px; min-width: 32px; padding: 1px 10px; border-radius: 999px; }
    .body { overflow: visible; }
    fieldset { border: 1px solid rgba(0,0,0,.08); margin: 10px 0; padding: 10px; border-radius: 18px; background: rgba(255,255,255,.72); }
    ...
    input[type="text"], input[type="url"], input[type="number"], input[type="datetime-local"], select { width: 100%; min-width: 0; height: 30px; border: 1px solid #d2d2d7; border-radius: 10px; padding: 3px 8px; background: rgba(255,255,255,.92); color: #1d1d1f; }
    button { white-space: nowrap; padding: 5px 14px; min-height: 30px; border: 1px solid rgba(0,0,0,.12); border-radius: 999px; background: #f5f5f7; color: #1d1d1f; cursor: var(--jdbean-cursor-hand), pointer; }
    ...
    button.btn-loading { background: #e6f0ff; color: #0a84ff; border-color: #b4d2ff; opacity: 1; }
    button.btn-loading::after { content: ''; display: inline-block; width: 1em; text-align: left; animation: jdbean-loading-dots 1.2s steps(1, end) infinite; }
    @keyframes jdbean-loading-dots { 0%{content:'';} 25%{content:'.';} 50%{content:'..';} 75%,100%{content:'...';} }
    .detect-status.loading { color: #0a84ff; font-weight: 600; }
    .window-dots button.window-dot { width: 12px; height: 12px; min-width: 12px; min-height: 12px; padding: 0; border: 0; border-radius: 50%; box-shadow: inset 0 0 0 1px rgba(0,0,0,.08); cursor: var(--jdbean-cursor-hand), pointer; line-height: 1; }
    ...
    .dot-close { background: #ff5f57; } .dot-minimize { background: #ffbd2e; } .dot-zoom { background: #28c840; }
    .panel.maximized { top: 12px !important; left: 12px !important; transform: none !important; width: calc(100vw - 24px) !important; max-width: none !important; max-height: calc(100vh - 24px) !important; }
    .restore-pill { position: fixed; top: 16px; right: 16px; z-index: 2147483647; padding: 8px 14px; border-radius: 999px; box-shadow: 0 12px 36px rgba(0,0,0,.18); background: #fff; }
    .stats { display: grid; grid-template-columns: repeat(6, minmax(0, 1fr)); gap: 8px; margin-bottom: 10px; }
    .stat { border: 1px solid rgba(0,0,0,.08); background: rgba(255,255,255,.78); padding: 10px 11px; min-height: 58px; border-radius: 18px; box-shadow: 0 1px 2px rgba(0,0,0,.04); }
    ...
    .stat-hit { background: #effaf2; border-color: #b9e7c3; } .stat-hit b { color: #137333; }
    .stat-nohit { background: #fff6e8; border-color: #ffd9a3; } .stat-nohit b { color: #a15c00; }
    .stat-error { background: #fff0f0; border-color: #ffc9c9; } .stat-error b { color: #b3261e; }
    ...
    .table-wrap { max-height: 430px; max-width: 100%; overflow: auto; border: 1px solid #e5e5ea; border-radius: 16px; background: #fff; }
    .table-wrap table.result-table { table-layout: fixed; width: max-content; min-width: 100%; border-collapse: separate; }
    .table-wrap table th { position: sticky; top: 0; z-index: 1; background: #f5f5f7; white-space: nowrap; }
    .table-wrap table th, .table-wrap table td { height: 38px; max-height: 38px; padding: 6px 8px; vertical-align: top; }
    .result-cell { display: block; max-width: 100%; overflow: hidden; text-overflow: ellipsis; white-space: nowrap; line-height: 1.4; }
    .result-cell-multiline { display: -webkit-box; -webkit-box-orient: vertical; -webkit-line-clamp: 2; max-height: 38px; overflow: hidden; white-space: normal; overflow-wrap: anywhere; line-height: 1.35; }
    .table-wrap tbody tr { height: 38px; }
    .col-status { width: 84px; } .col-event { width: 132px; } .col-tracker { width: 96px; } .col-erp { width: 110px; } .col-account { width: 150px; } .col-time { width: 158px; } .col-detail { width: 320px; }
    ...
    .th-filter { flex: 0 0 auto; width: 18px; height: 18px; min-height: 0; padding: 0; border-radius: 6px; border: 1px solid transparent; background: transparent; color: #6e6e73; font-size: 11px; line-height: 1; display: inline-flex; align-items: center; justify-content: center; }
    ...
    .filter-popover { position: fixed; z-index: 2147483647; width: 240px; max-height: 360px; background: #fff; border: 1px solid rgba(0,0,0,.12); border-radius: 12px; box-shadow: 0 12px 36px rgba(0,0,0,.18); padding: 10px; display: flex; flex-direction: column; gap: 8px; font-size: 12px; }
    .filter-popover-title { font-weight: 700; color: #1d1d1f; }
    .filter-popover input[type="text"] { height: 28px; }
    .filter-list { flex: 1 1 auto; overflow-y: auto; border: 1px solid #e5e5ea; border-radius: 8px; background: #fbfbfd; padding: 4px 0; max-height: 220px; }
    .filter-item { display: flex; align-items: center; gap: 6px; padding: 4px 8px; cursor: var(--jdbean-cursor-hand), pointer; white-space: nowrap; }
    .filter-item:hover { background: #eef4ff; }
    .filter-item input[type="checkbox"] { flex: 0 0 auto; margin: 0; }
    .filter-item-text { flex: 1 1 auto; overflow: hidden; text-overflow: ellipsis; white-space: nowrap; }
  ...
  <div class="panel" id="panel">
    <div class="runtime-title">
      <span class="runtime-title-text" id="runtimeTitle">SmsQueryTool｜运行环境：Chrome / Edge（Chromium）扩展，Manifest V3</span>
    </div>
    <div class="header" id="dragHandle">
      <div class="brand">
        ...
        <div>
          <div class="title">SmsQueryTool</div>
          <div class="sub">CRM → 来电号码 → SMS</div>
        </div>
      </div>
      ...
      <fieldset>
        ...
        <table class="form-table">
          <tr>
            ...
            <td style="width:230px;">
              <div style="display:flex;gap:8px;align-items:center;">
                <select id="sourceChannel" style="flex:0 0 auto;width:110px;">
          ...
          <tr id="savedCrmRow" class="hidden">
            ...
            <td colspan="3">
              <div style="display:flex;gap:8px;align-items:center;">
                <select id="savedCrmSelect" style="flex:1 1 auto;"><option value="">—— 选择已保存的来源 ——</option></select>
                <button id="deleteCrmBtn" type="button" style="flex:0 0 auto;" disabled>🗑 删除</button>
              </div>
            </td>
          </tr>
          <tr id="customUrlRow" class="hidden">
            <th>来源URL</th>
            <td colspan="3"><input type="url" id="customSourceUrl" placeholder="粘贴其他组的 CRM 数据来源URL（含 monitorDetail?...&parDeptId=...）" /></td>
          </tr>
          <tr id="crmNameRow" class="hidden">
            <th>命名保存</th>
            <td colspan="3">
              <div style="display:flex;gap:8px;align-items:center;">
                <input type="text" id="crmNameInput" maxlength="40" placeholder="给这个来源起个名字，例如「华东二组」" style="flex:1 1 auto;" />
                <button id="saveCrmBtn" type="button" style="flex:0 0 auto;">💾 保存到本地</button>
              </div>
            </td>
          </tr>
          <tr>
            <td colspan="4"><span class="detect-status" id="detectStatus">等待读取CRM数据。</span></td>
          </tr>
        </table>
          ...
          <table class="result-table">
            ...
            <thead>
              <tr>
                ...
                <th data-col-key="smsSendTime"><div class="th-cell"><span class="th-text">短信发送时间</span><button type="button" class="th-filter" data-col-key="smsSendTime" title="筛选" aria-label="筛选 短信发送时间">▾</button></div></th>
