...
async function loadCrmSourceData() {
  ...
  try {
    ...
    const rows = creatorInfo.rows.map(row => {
      const copy = { ...row };
      const rawTracker = trackerCol ? clean(row[trackerCol]) : '';
      copy.__trackerRaw = rawTracker;
      copy.__trackerName = rawTracker ? extractTrackerChineseName(rawTracker) : '';
      copy.__trackerErp = rawTracker ? extractTrackerErp(rawTracker) : '';
      return copy;
    });
    state.crmData = {
      label,
...
async function fetchCrmRowsFromDetailUrl(detailUrl, onProgress) {
  ...
  // 第 1 页已经拿到；剩下的并发抓，按页号顺序拼接。
  const CRM_PAGE_CONCURRENCY = 6;
  const limiter = createAsyncLimiter(CRM_PAGE_CONCURRENCY);
  let done = 1;
  ...
  for (let page = 2; page <= pages; page++) {
    ...
    restPromises.push(limiter(async () => {
      const pageHtml = await fetchCrmMonitorPage(params, p, pageSize, detail.origin);
      const pageDoc = new DOMParser().parseFromString(pageHtml, 'text/html');
      const table = findCrmDataTable(pageDoc);
      if (!table) throw new Error(`CRM第 ${p} 页返回中未找到可识别的数据表格`);
      const parsed = parseCrmMonitorTableObjects(table);
      done++;
      onProgress && onProgress(done, pages, total);
...
function getTrackerErpFromRow(row) {
  const cachedRaw = clean(row?.__trackerRaw);
  if (cachedRaw) return extractTrackerErp(cachedRaw);
  const raw = getRowValueByCandidates(row, TRACKER_COL_CANDIDATES);
...
function normalizeCrmCaseCloseDetailUrl(url, beginTimeStr) {
  ...
  target.searchParams.set('funName', CRM_FUN_NAME);
